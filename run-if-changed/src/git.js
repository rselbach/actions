...
import { ensureSafeDirectory, getGitEnv, isDubiousOwnershipError } from './safe-directory.js'
...
export async function getChangesInLastCommit() {
...
export async function getChanges(base, head) {
...
export async function getChangesOnHead() {
...
export async function getChangesSinceMergeBase(base, head, initialFetchDepth) {
  let baseRef
  let headRef
  async function hasMergeBase() {
    ...
    return (
      (
        await gitExec(['merge-base', baseRef, headRef], {
          ignoreReturnCode: true
        })
      ).exitCode === 0
    )
...
export async function listAllFilesAsAdded() {
...
export async function getCurrentRef() {
  ...
  try {
    ...
    const describe = await gitExec(['describe', '--tags', '--exact-match'], {
      ignoreReturnCode: true
    })
...
export function getShortName(ref) {
  if (!ref) return ''
...
export function isGitSha(ref) {
  return /^[a-z0-9]{40}$/.test(ref)
}
async function hasCommit(ref) {
  return (
    (
      await gitExec(['cat-file', '-e', `${ref}^{commit}`], {
        ignoreReturnCode: true
      })
    ).exitCode === 0
  )
