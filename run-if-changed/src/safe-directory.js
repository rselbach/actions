...
export function isDubiousOwnershipError(exitCode, stderr) {
  return exitCode === 128 && DUBIOUS_OWNERSHIP_PATTERN.test(stderr)
}
export function parseRepositoryPath(stderr) {
...
export async function ensureSafeDirectory(stderr) {
...
// Exported for tests
export async function createTempGitHome(baseTempDir, env) {
  const tempHome = await fs.promises.mkdtemp(path.join(baseTempDir, 'run-if-changed-git-home-'))
...
// Exported for tests
export function buildGitEnv(tempHome, env) {
...
// Exported for tests
export function resolveTempBaseDir(env) {
