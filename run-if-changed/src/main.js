...
import { isPredicateQuantifier, Filter, PredicateQuantifier, SUPPORTED_PREDICATE_QUANTIFIERS } from './filter.js'
import { ChangeStatus } from './file.js'
import * as git from './git.js'
import { cleanup as cleanupSafeDirectory } from './safe-directory.js'
    ...
    for await (const response of client.paginate.iterator(
      ...
      for (const row of response.data) {
        ...
        if (row.status === ChangeStatus.Renamed) {
          ...
          files.push({
            // 'previous_filename' is not documented by this endpoint
